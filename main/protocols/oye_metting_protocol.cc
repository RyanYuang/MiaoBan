#include "oye_metting_protocol.h"
...
// 构造函数
OyeMettingProtocol::OyeMettingProtocol() {
...
// 设置主调度器
void OyeMettingProtocol::SetMainScheduler(std::function<void(std::function<void()>)> scheduler) {
    schedule_ = std::move(scheduler);
}
// 启动协议
bool OyeMettingProtocol::Start() {
    return true;
}
// 打开音频通道
bool OyeMettingProtocol::OpenAudioChannel() {
...
//关闭音频通道
void OyeMettingProtocol::CloseAudioChannel(bool send_goodbye) {
    (void)send_goodbye;
...
// 确认音频通道是否打开
bool OyeMettingProtocol::IsAudioChannelOpened() const {
    return channel_open_ && !error_occurred_;
}
// 确认语音结果是否准备好
bool OyeMettingProtocol::IsVoiceResultPending() const {
    return processing_.load();
}
// 发送语音包
bool OyeMettingProtocol::SendAudio(std::unique_ptr<AudioStreamPacket> packet) {
    (void)packet;
    return true;
}
// 发送文本
bool OyeMettingProtocol::SendText(const std::string& text) {
    (void)text;
    return true;
}

void OyeMettingProtocol::SendAbortSpeaking(AbortReason reason) {
 ...
 */
void OyeMettingProtocol::SendStartListening(ListeningMode mode) {
...
void OyeMettingProtocol::SendStopListening() {
...
// 处理语音结果任务入口
void OyeMettingProtocol::ProcessUtteranceEntry(void* arg) {
    static_cast<OyeMettingProtocol*>(arg)->ProcessUtteranceTask(arg);
}
// 处理语音结果任务
void OyeMettingProtocol::ProcessUtteranceTask(void* arg) {
    (void)arg;
...
// 发送JSON
void OyeMettingProtocol::EmitJson(const char* type, const std::string& text, const char* state) {
